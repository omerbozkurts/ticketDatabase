...
mevcut_biletler = pd.read_excel('uniqueChecker.xlsx')  # Dosya adını uygun şekilde değiştirin
...
# Unique key oluşturmak için gerekli sütunları al
for _, row in mevcut_biletler.iterrows():
    unique_checker.add((row['KoltukNo'], row['Firma'], row['Sefer'], row['Arac']))
